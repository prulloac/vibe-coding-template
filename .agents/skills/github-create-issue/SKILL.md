...
1. Suggest bug template if available
1. Create the issue using the best available tool
...
1. **GitHub CLI** (`gh` command, requires authentication)
1. **GitHub REST API** (requires `GITHUB_TOKEN` environment variable)
...
1. Scores templates by keyword match (looks for words in template names/metadata)
1. If strong match found (e.g., template named "bug-report"), suggests it
1. Creates issue with selected template
...
1. Displays options: "Which template? 1) Enhancement 2) Feature Request 3) Other"
1. Uses selected template
1. Creates issue
...
1. Matches based on template name and metadata
1. Suggests best match or presents options
1. Creates issue with matched template
...
1. Creates issue directly with just title and body
...
1. Attempts GitHub REST API
1. Checks for `GITHUB_TOKEN` environment variable
1. Falls back with error if neither tool available
...
1. Match templates based on their names and metadata (not hardcoded categories)
1. Suggest best matching template or present options
1. Create the issue using available tools
1. Return issue URL or error details
