...
1. Add the standard "Keep a Changelog" header.
1. Identify all tags and their changes to build the history, or start from the latest tag.
...
1. **Format**: Check that the file includes the required header and that new changes are correctly categorized under the appropriate heading (e.g., `### Added`).
1. **Links**: If the file uses links at the bottom for versions, ensure they point to valid git comparison URLs (if applicable).
1. **SemVer**: Ensure the version numbers used follow the SemVer specification.
