...
1. Uses the first available tool
1. Creates the label with provided parameters
1. Returns success confirmation
...
1. Creates each label using available tool
1. Returns list of created labels or errors
...
1. Creates "enhancement" label (cyan)
1. Creates "documentation" label (blue)
1. Creates "help wanted" label (dark green)
...
1. Uses available tool to create
1. Returns confirmation
...
1. Uses REST API if available
1. Returns error if no tools available
...
1. Creates each label using available tool
1. Tracks successes and failures
...
1. Create each label using the best available tool
1. Return confirmation for each created label
1. Report any errors that occur

## Best Practices

1. **Consistency**: Use consistent naming across projects (lowercase, hyphens for spaces)
1. **Documentation**: Always include descriptions for clarity
1. **Colors**: Use colors to create visual patterns (red for bugs, green for help, blue for docs)
1. **Batch Creation**: Create all needed labels upfront during project setup
1. **Naming Conventions**: Follow repository guidelines for label naming
