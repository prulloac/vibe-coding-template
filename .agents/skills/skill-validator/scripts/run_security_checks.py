...
def audit_skill(skill_path: Path, auditor: SecurityAuditor) -> Dict[str, Any]:
    ...
    for script_name, script_content in scripts:
        script_result = auditor.audit(script_content, str(skill_path / "scripts" / script_name))
...
def aggregate_results(audit_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    scripts_passed = all(
        result.passed 
        for result in audit_data['script_audits'].values()
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run security checks on a skill"
    )
